...
pub enum MembershipError {
    ...
    #[error(
        "quorum required: need {required} signatures, got {provided}"
    )]
    ...
    #[error(
        "host posture level {current} is below the minimum floor {floor}"
    )]
    ...
    #[error(
        "coordinator invitation not found for membership {membership_id}"
    )]
