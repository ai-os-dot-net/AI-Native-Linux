...
impl AppArmorProfileGenerator {
    ...
    fn write_header(&self, buf: &mut String, profile_name: &str, sandbox: &SandboxProfile) {
        ...
        buf.push_str(&format!("# Profile: {name}\n", name = sandbox.name));
        buf.push_str(&format!(
            "# Description: {desc}\n",
    ...
    fn write_path_rules(&self, buf: &mut String, profile_name: &str) {
        ...
        buf.push_str("\n  # ── Capsule home ─────────────────────────\n");
        buf.push_str(&format!("  owner {capsule_home} r,\n",));
        buf.push_str(&format!("  owner {capsule_home}** rwlkmix,\n",));
...
mod tests {
    ...
    fn profile_contains_capsule_path_rules() {
        ...
        assert!(result.content.contains("/var/lib/aios/capsules/aios-"));
